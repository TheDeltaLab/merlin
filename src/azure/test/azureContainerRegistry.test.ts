...
const CONDITIONAL_PUSH_TARGET_ARG_INDEX = 6;
...
describe('AzureContainerRegistryRender', () => {
    ...
    describe('images - source pull/tag/push', () => {
        it('generates az acr login then docker pull/tag/push per tag for source image', async () => {
            ...
            // conditional push per tag
            const pushCmds = commands.filter(c => c.command === 'bash' && c.args[0] === '-c');
            expect(pushCmds).toHaveLength(2);
            expect(pushCmds[0].args[CONDITIONAL_PUSH_TARGET_ARG_INDEX]).toBe('myprojectmyacrstgeusacr.azurecr.io/nginx:latest');
            expect(pushCmds[1].args[CONDITIONAL_PUSH_TARGET_ARG_INDEX]).toBe('myprojectmyacrstgeusacr.azurecr.io/nginx:v1.0');
        });
        ...
        it('generates pull/tag/push for multiple source images with a single acr login', async () => {
            ...
            expect(pushCmds[0].args[CONDITIONAL_PUSH_TARGET_ARG_INDEX]).toBe('myprojectmyacrstgeusacr.azurecr.io/nginx:latest');
            expect(pushCmds[1].args[CONDITIONAL_PUSH_TARGET_ARG_INDEX]).toBe('myprojectmyacrstgeusacr.azurecr.io/redis:7.0');
            expect(pushCmds[2].args[CONDITIONAL_PUSH_TARGET_ARG_INDEX]).toBe('myprojectmyacrstgeusacr.azurecr.io/redis:7.2');
        });
    });

    describe('images - generateScript build', () => {
        it('generates az acr login + bash + docker tag + conditional push for single tag', async () => {
            mockNotFound();
            const resource = makeResource({
                images: [
                    {
            ...
            const bashCmds = commands.filter(c => c.command === 'bash' && c.args[0] !== '-c');
            const tagCmds = commands.filter(c => c.command === 'docker' && c.args[0] === 'tag');
            const pushCmds = commands.filter(c => c.command === 'bash' && c.args[0] === '-c');
            ...
            expect(pushCmds[0].args[CONDITIONAL_PUSH_TARGET_ARG_INDEX]).toBe('myprojectmyacrstgeusacr.azurecr.io/customapp:latest');
        });
        ...
        it('uses correct loginServer format <registryName>.azurecr.io', async () => {
            ...
            const pushCmds = commands.filter(c => c.command === 'bash' && c.args[0] === '-c');
            expect(pushCmds[0].args[CONDITIONAL_PUSH_TARGET_ARG_INDEX]).toContain(expectedLoginServer);
        });
    });
    ...
    describe('images - mixed source and generateScript', () => {
        it('handles multiple images with different types in order', async () => {
            ...
            // 1 az acr login + (1 pull + 1 tag + 1 conditional push for nginx) + 1 bash + 1 tag + 1 conditional push for customapp = 7
            ...
            expect(imageCommands[3].command).toBe('bash');       // conditional push .../nginx:latest
            expect(imageCommands[3].args[0]).toBe('-c');
            ...
            expect(imageCommands[6].command).toBe('bash');       // conditional push .../customapp:v1
            expect(imageCommands[6].args[0]).toBe('-c');
        });
